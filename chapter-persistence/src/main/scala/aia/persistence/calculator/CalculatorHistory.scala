...
class CalculatorHistory extends Actor {
  ...
  def receive = {
    case Added => history = history.incrementAdded
    case Subtracted => history = history.incrementSubtracted
    case Divided => history = history.incrementDivided
    case Multiplied => history = history.incrementMultiplied
